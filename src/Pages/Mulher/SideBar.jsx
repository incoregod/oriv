...
export default function SideBar({ type }) {
  ...
  const { produtos, categorias } = useContext(CategoriesContext);
        ...
        <main className="p-5">
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className="grid grid-cols-6 gap-5">
              ...
              <div className="lg:col-span-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:col-span-5 col-span-full grid grid-cols-1 gap-x-8 gap-y-10  grid-rows-3">
                {/* Product grid */}
                {produtos.map((item) => (
                  <div key={item._id} className="">
                    <img className="w-full" src={urlFor(item.imgUrl)} />
                  </div>
                ))}
              </div>
            </div>
